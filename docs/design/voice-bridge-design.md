...
**Why `VOICE_BRIDGE_GCP_KEY_PATH` and NOT `GOOGLE_APPLICATION_CREDENTIALS`**: the bridge process also runs the Anthropic Agent SDK in Vertex mode (`CLAUDE_CODE_USE_VERTEX=1`). The Anthropic SDK reads `GOOGLE_APPLICATION_CREDENTIALS` from process.env to authenticate Claude→Vertex calls. Using the standard name would cause the Anthropic SDK to authenticate as the personal voice-bridge SA (no permission on the the corporate Vertex project) and crash every Claude turn with `aiplatform.endpoints.predict denied`. By using a bridge-namespaced var and passing the path explicitly to STT/TTS clients via `keyFilename`, the Anthropic SDK falls back to ADC (the corporate account) as intended.
