...
async function tryVoice(name: string, text: string): Promise<void> {
  try {
    ...
    console.log(
      `FAIL ${name.padEnd(30)} code=${err.code} msg="${(err.message ?? '').slice(0, 150)}"`,
    );
  }
}
...
await tryVoice(
  ...
  'Σάββατο, σχετικά ήσυχη μέρα. Έχεις την τελευταία μέρα του Delphi Economic Forum, που είναι όλη μέρα. Στις εννέα με έντεκα το πρωί έχεις μπλοκάρει focus time, και στη μία και μισή το μεσημέρι το lunch break σου.',
);
