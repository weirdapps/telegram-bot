...
21. **Confidence-zero handling in STT defeats the `MIN_CONFIDENCE = 0.3` threshold for non-chirp models (bridge component, fixed 2026-04-24).**
...
22. **Speech v2 multi-language auto-detect only available in `eu`/`global`/`us` multi-regions (bridge component, design constraint).**
...
23. **Speech v2 `chirp_2` model not deployed in any multi-region (bridge component, design bug, fixed 2026-04-24).**
...
24. **Pre-existing strict-mode latent issue in `bridge/src/claude.ts` (only surfaces when bridge files are explicitly typechecked).**
...
25. **Bridge LaunchAgent default install fails on CloudStorage-backed source paths (bridge component).**
...
27. **GramJS `_updateLoop` race mitigation is a workaround, not a fix (ADR-009).**
...
28. **GramJS version pinned to `^2.26.22`.**
...
12. **Design-doc drift — incoming-media filename scheme.**
...
13. **Design-doc gap — facade `logout()` method not documented in §4.11.**
