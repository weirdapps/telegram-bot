...
1. **Unattended 2FA login requires the password in a file (OOS-14 / ADR-006).**
...
1. **`GOOGLE_APPLICATION_CREDENTIALS` env-var collision between Google Cloud SDKs and the Anthropic Vertex SDK (bridge component, fixed 2026-04-24).**
   The original voice-bridge spec (`voice-bridge-design.md` §5) used `GOOGLE_APPLICATION_CREDENTIALS` as the bridge's path to its Speech/TTS SA key. Problem: when the bridge ALSO runs the Anthropic Agent SDK in Vertex mode (`CLAUDE_CODE_USE_VERTEX=1`), the SDK reads the same env var to authenticate Claude→Vertex calls. Result: Claude calls authenticate as the personal voice-bridge SA (no permission on the corporate Vertex project) and crash every turn with `aiplatform.endpoints.predict denied on resource '//.../projects/<corp-project>'`. Workaround in place: rename to `VOICE_BRIDGE_GCP_KEY_PATH` and pass the path explicitly to `createSpeechClient(projectId, keyFilename)` and `createTtsClient(projectId, keyFilename)` so the SDK never sees the bridge's credential. General lesson: never reuse standard env-var names with hard-coded SDK semantics in a process that hosts multiple SDKs.

2. **Confidence-zero handling in STT defeats the `MIN_CONFIDENCE = 0.3` threshold for non-chirp models (bridge component, fixed 2026-04-24).**
...
3. **Speech v2 multi-language auto-detect only available in `eu`/`global`/`us` multi-regions (bridge component, design constraint).**
...
4. **Speech v2 `chirp_2` model not deployed in any multi-region (bridge component, design bug, fixed 2026-04-24).**
...
5. **Pre-existing strict-mode latent issue in `bridge/src/claude.ts` (only surfaces when bridge files are explicitly typechecked).**
...
6. **Bridge LaunchAgent default install fails on CloudStorage-backed source paths (bridge component).**
...
8. **GramJS `_updateLoop` race mitigation is a workaround, not a fix (ADR-009).**
...
9. **GramJS version pinned to `^2.26.22`.**
...
1. **Peer resolution cache is unbounded in-process.**
...
2. **No edit / delete / reaction / poll support.**
   Out of scope for v1.

3. **No secret-chat (E2E) support.**
   GramJS does not expose secret chats the same way as normal messages; out of scope for v1.

4. **Transitive deprecation warnings emitted by `npm install` (unavoidable until upstream updates).**
...
5. **Design-doc drift — incoming-media filename scheme.**
...
6. **Design-doc gap — facade `logout()` method not documented in §4.11.**
